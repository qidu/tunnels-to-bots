...
class TunnelServer {
  ...
  private handleMessage(ws: ExtendedWS, data: string): void {
    try {
      const frame = JSON.parse(data) as { type: string; data: Record<string, unknown>; id?: string };
      ws.lastActivity = Date.now();
  ...
  private handleAuth(ws: ExtendedWS, frame: { data: { apiKey?: string; token?: string } }): void {
  ...
  private handleMessageFrame(ws: ExtendedWS, frame: { data: { to: string; text?: string } }): void {
    if (!ws.userId) {
  ...
  private handleTask(ws: ExtendedWS, frame: { data: { title: string; description: string; priority: string; botId: string }; id?: string }): void {
    if (!ws.userId) {
      this.sendError(ws, 401, 'Not authenticated');
      return;
    }
  ...
  private handleSubscribe(ws: ExtendedWS, frame: { data: { botId: string } }): void {
  ...
  private handleStatusRequest(ws: ExtendedWS, frame: { data: { request: string } }): void {
  ...
  private sendError(ws: ExtendedWS, code: number, message: string): void {
...
function registerBot(userId: string, name: string, type: string): Bot {
...
function getUserBots(userId: string): Bot[] {
